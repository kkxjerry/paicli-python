...
> Java → Python 的行为同步是独立迁移线，不改变下面原有的 22 期编号。Phase 0-8 的基线、差异和验收项见 `docs/java-parity.md`；实现留痕见 `docs/phase-03-05-implementation.md` 与 `docs/phase-06-08-implementation.md`。
...
- `paicli/planning.py`：`LlmPlanner + PlanValidator + DagScheduler` 统一计划合同与只读任务批次并发。
- `paicli/subagents.py`、`review.py`、`orchestration.py`：真实 LLM Worker、结构化 Reviewer、局部重试和 Plan/Team Runtime。
- `paicli/__main__.py`：`--mode react|plan|team`、`/plan` 与 `/team` 的实际 CLI 入口。
